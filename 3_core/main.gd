...
const SCENE_MAP = preload("res://3_core/map.tscn")
const SCENE_START_LIGHT = preload("res://entities/start_light.tscn")
...
func start_level():
	...
	if globals.depth > 1: 
		...
		#	globals.mapold  = $Map.duplicate()
		#	add_child(globals.mapold)
	...
	var start_light = SCENE_START_LIGHT.instance()
	
	start_light.position.x = globals.player.position.x
	start_light.position.y = globals.player.position.y
	add_child(start_light)
