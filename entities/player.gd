...
func _physics_process(delta: float):
	...
	# velocity reduced by factor / 3 for 640x360 mode
	_velocity = direction  * base_speed / 2
...
func take_damage(collision_dir: Vector2, damage: float, factor: float):
	...
	if health <= 0:
		get_tree().change_scene("res://3_core/game-over.tscn")	
...
func _attack():
	...
	if _attack_dir.x > 0 and abs(_attack_dir.x) > abs(_attack_dir.y):
		weapon.rot = 90
		weapon.position.x = 8
		weapon.position.y = 16
	elif _attack_dir.x < 0 and abs(_attack_dir.x) > abs(_attack_dir.y):
		weapon.rot = -90
		weapon.position.x = 8
		weapon.position.y = 16
